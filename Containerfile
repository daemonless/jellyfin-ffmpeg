...
# Which overlay port this tag builds, and the versioned binary name it installs
# (the port is named jellyfin-ffmpeg but ships bin/jellyfin-ffmpeg8). Both set
# per variant in .daemonless/config.yaml; :8 carries the `latest` alias.
ARG JFFMPEG_PORT=jellyfin-ffmpeg
ARG JFFMPEG_BIN=jellyfin-ffmpeg8
...
RUN pkg install -y /tmp/pkgs/${JFFMPEG_PORT}-*.pkg && \
    rm -rf /tmp/pkgs && \
    pkg clean -ay && \
    ln -sf /usr/local/bin/${JFFMPEG_BIN} /usr/local/bin/ffmpeg && \
    ln -sf /usr/local/bin/$(echo ${JFFMPEG_BIN} | sed 's/ffmpeg/ffprobe/') /usr/local/bin/ffprobe && \
    mkdir -p /app && pkg query '%v' ${JFFMPEG_PORT} > /app/version

# CLI tool, not an s6 service. `ffmpeg` = this tag's jellyfin-ffmpeg line;
# the versioned binary (jellyfin-ffmpeg8) is also on PATH.
#   podman run --rm ghcr.io/daemonless/jellyfin-ffmpeg:latest -i in.mkv ... out.mp4
